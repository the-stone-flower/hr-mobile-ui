...
const ContactInfo: React.FC<TabProps> = ({ form, allOptions }) => {
  ...
  // 前端管理的地址字段
  const [familyRegion, setFamilyRegion] = useState<string[]>(['四川省', '成都市', '锦江区']);
  const [familyLocation, setFamilyLocation] = useState('');
  const [emergencyRegion, setEmergencyRegion] = useState<string[]>(['四川省', '成都市', '锦江区']);
  const [emergencyLocation, setEmergencyLocation] = useState('');
  const [postalRegion, setPostalRegion] = useState<string[]>(['四川省', '成都市', '锦江区']);
  ...
  // 处理现居地址的省市区选择
  const handleFamilyRegionChange = (val: PickerValue[], extend: PickerValueExtend) => {
  ...
  // 处理紧急联系人地址的省市区选择
  const handleEmergencyRegionChange = (val: PickerValue[], extend: PickerValueExtend) => {
