...
const SkillInfo: React.FC<TabProps> = ({ form, allOptions }) => {
  ...
  const handleSkillChange = (_: number, val: any) => {
    const selectedSkill = allOptions.skill_options?.options.find(
      (option: any) => option.id === val[0]
    );
  ...
  const getPickerDisplayText = (
    ...
    if (formValue) {
      ...
      const option = options?.find((opt) =>
        opt.id === actualValue || opt.value === actualValue
      );
    ...
    <>
      {localSkillList.map((_: any, index: number) => (
        <div key={index}>
          ...
          >
            <Picker
              columns={[allOptions.skill_options?.options.map((item: any) => ({ label: item.name, value: item.id })) || []]}
              onConfirm={(val) => handleSkillChange(index, val)}
            >
              {(value) =>
                getPickerDisplayText(
                  value,
                  allOptions.skill_options?.options.map((item: any) => ({ label: item.name, value: item.id, ...item })) || [],
                  "请选择技能名称",
            ...
            <Picker columns={[skillLevelOptions]}>
              {(value) =>
                getPickerDisplayText(
                  ...
                  skillLevelOptions,
                  "请选择技能等级",
                  ["skill_info_list", String(index), "skill_level"]
                )
              }
            </Picker>
          </Form.Item>
          ...
          >
            ...
            >
              {(value) =>
                getDateDisplayText(
                  value,
                  ["skill_info_list", String(index), "skill_date"],
                  "请选择鉴定时间"
                )
              }
            </DatePicker>
